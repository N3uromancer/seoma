...
import network.client.ClientWriterThread;
...
public class World implements KeyListener
{
	 ...
	 * @param cwt the writer thread that sends data to the server
	 */
	public void updateWorld(double tdiff, ClientWriterThread cwt)
	{
		time+=tdiff;
		try
		{
			...
			for(int i = 0; i < 1; i++)
			{
				...
				System.arraycopy(baos.toByteArray(), 0, dest, 0, temp.length);
				cwt.add(dest, obj.get(avatarID));
			}
		}
