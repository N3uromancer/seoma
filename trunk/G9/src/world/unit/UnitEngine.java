...
import world.World;
import world.animation.animations.UnitExplosion;
...
{
	PartitionManager ausp; //all units spatial partition, for drawing
	HashMap<Owner, PartitionManager> usp = new HashMap<Owner, PartitionManager>(); //unit spatial partitions
	...
	public UnitEngine(World w)
	{
		int minItems = 10;
		int maxItems = 100;
		int minPartitionArea = 10;
		
		ausp = new PartitionManager(0, 0, w.getMapWidth(), w.getMapHeight(), minItems, maxItems, minPartitionArea);
		this.w = w;
		...
		for(int i = 0; i < o.length; i++)
		{
			...
			usp.put(o[i], new PartitionManager(0, 0, w.getMapWidth(), w.getMapHeight(), minItems, maxItems, minPartitionArea));
	...
	public HashMap<Owner, PartitionManager> getUnits()
	{
		return usp;
	}
	/**
	...
	{
		//usp.addRegion(unit);
		usp.get(unit.getOwner()).add(unit);
		ausp.add(unit);
	...
	public void updateUnitEngine(double tdiff, World w)
	{
		...
		while(i.hasNext())
		{
			...
			while(ui.hasNext())
			{
				...
				if(unit.isDead())
				{
					...
					usp.get(unit.getOwner()).remove(unit);
					ausp.remove(unit);
					ui.remove();
					w.getAIs().get(unit.getOwner()).unitDestroyed(unit);
		...
		{
			System.out.println("total unit engine updates = "+totalUpdates);
	 ...
	 */
	public PartitionManager getUnitPartition(Owner o)
	{
		return usp.get(o);
	}
	 ...
	 * @return
	 */
	public PartitionManager getAllUnits()
	{
		return ausp;
	}
	public void drawAll(double x, double y, double width, double height, GL gl)
	{
		HashSet<Boundable> r = ausp.intersects(x, y, width, height);
