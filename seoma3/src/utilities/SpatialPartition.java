...
public final class SpatialPartition extends RectRegion
{
	 ...
	 * @return returns all regions that intersect with the passed region
	 */
	public HashSet<Region> getIntersections(RectRegion region)
	{
	...
	public HashSet<Region> getRegions(double x, double y, double radius)
	{
		...
		while(i.hasNext())
		{
			...
			if(!regions.contains(rtemp) && MathUtil.distance(x, y, rtemp.getLocation()[0]+rtemp.getWidth()/2, rtemp.getLocation()[1]+rtemp.getHeight()/2) <= radius)
			{
				regions.add(rtemp);
			}
		}
