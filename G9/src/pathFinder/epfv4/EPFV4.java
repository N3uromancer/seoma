 ...
 *
 */
public final class EPFV4 extends PathFinder
{
	...
	public EPFV4(int width, int height, Polygon[] p)
	{
	...
	private ArrayList<HashSet<SNode>> findIslands(SNode[] n)
	{
		...
		for(int i = 0; i < n.length; i++)
		{
			...
			if(!contained)
			{
				...
				HashSet<SNode> toAdd = new HashSet<SNode>();
				SNode[] adj = n[i].getAdjacentNodes();
				for(int q = 0; q < adj.length; q++)
				{
					toAdd.add(adj[q]);
				}
				HashSet<SNode> toAddTemp = new HashSet<SNode>();
				while(toAdd.size() > 0)
				{
					for(SNode s: toAdd)
					{
						...
						temp.add(s);
						for(SNode adjNode: s.getAdjacentNodes())
						{
							//System.out.println("here2");
							if(!temp.contains(adjNode) && !toAdd.contains(adjNode))
							{
								//System.out.println("here3");
								toAddTemp.add(adjNode);
							}
						}
					}
					//System.out.println("here4");
					toAdd = toAddTemp;
					toAddTemp = new HashSet<SNode>();
				}
